...
src/
  ...
  config.zig         结构化配置（backend / agent / tools / skills / audit）；TOML 优先 / JSON 回落
  toml.zig           自研零依赖 TOML 子集解析器（→ std.json.Value，复用 JSON 类型映射）
...
```text
~/.scoot/
  config.toml   主配置（推荐，可读性更好；也支持 config.json）
...
配置支持 **TOML（推荐，可读性更好、可写注释）** 与 **JSON** 两种格式，**`config.toml` 优先于 `config.json`**；两者字段、默认值、合并语义完全一致。

`config.toml`（推荐）：

```toml
# 缺省即用内置默认值；只需写出想覆盖的字段
[backend]
base_url    = "http://127.0.0.1:11434/v1"
model       = "qwen2.5"
api_key_env = "OPENAI_API_KEY"
# api_key_file / api_key_cmd / ca_file 可选

# 动态扩展请求体参数（透传进 chat/completions 顶层），如 Azure / 推理模型
[backend.extra_body]
...
```json
{
  ...
  "tools":  { "timeout_ms": 30000, "policy": "guarded" },
...
加载语义：**`config.toml` 优先，否则回落 `config.json`，皆缺失则全用默认**。**按节按字段合并**——只需写出想覆盖的字段，未写出的回落内置默认；未知字段忽略（向后兼容）；TOML/JSON 畸形则报出文件路径并拒绝启动（坏配置可见，不静默吞掉）。`SCOOT_HOME` 可整体改写运行目录。TOML 解析为零依赖自研子集（覆盖表 / 表数组 / 行内表 / 数组 / 注释；不支持日期时间、多行串、非十进制整数——遇到即明确报错）。
...
> **扩展参数（`backend.extra_body`）：** 默认 `null`。一个**透传 JSON 对象**，其成员被原样合并进 `chat/completions` 请求体顶层——用于后端特有 / 新增字段，**无需为每个参数加代码**（对齐 OpenAI SDK 的 `extra_body`）。典型场景：Azure 的 `service_tier`、推理模型的 `reasoning_effort`、`top_p` 等。仅接受 JSON 对象（非对象忽略，防弹）；**明文密钥严禁放这里**（密钥只走下方三来源）。同名键由 `extra_body` 覆盖，故请勿在此重定义 `model` / `messages` 等核心字段。
>
> Azure（priority 通道、推理高强度）示例——密钥仍走环境变量，绝不内联：
> ```toml
> [backend]
...
**默认绝不把明文密钥写进配置文件（`config.toml` / `config.json`）或随仓库提交；token 只在内存中短暂存活，且绝不写进日志 / 审计。** 解析优先级（高 → 低）：
...
3. **凭证命令**（`backend.api_key_cmd`，如 `pass show openai`）—— 其标准输出即 token，借助 `pass` / `gpg` / 钥匙串 CLI 等外部工具实现安全存储，**不引入平台钥匙串依赖**；命令受 **10s 硬超时**约束（坏命令不拖死启动）。

> 三种来源**均已落地**：按上述优先级逐个尝试，命中即用。token 文件存在但权限过宽会**拒绝读取并提示 `chmod 600`**（绝不把世界可读的密钥读进内存）；显式配了文件/命令来源却全部落空时明示告警；本地无须密钥的后端（如 Ollama）则静默以空 token 续跑。config 内联明文 `api_key` **刻意不提供**，以杜绝密钥随仓库提交。
...
| 配置加载 | `src/config.zig` · `src/toml.zig` | ✅ `~/.scoot/config.toml`（优先，可读性更好）/ `config.json`（回落）；自研零依赖 TOML 子集解析→`std.json.Value`，再经 `parseFromValueLeaky` **复用 JSON 的类型映射 / 按节合并 / 默认回落 / 畸形清晰报错**（含测试）；内联明文密钥字段**刻意不提供**（杜绝随仓库提交） |
