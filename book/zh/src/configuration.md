...
| `SCOOT_AUDIT_MAX_RETAINED_GENERATIONS` | `audit.max_retained_generations` | 整数 |
| `SCOOT_EDGE_MAX_JOB_POLICY` | `edge.max_job_policy` | 字符串（`guarded`/`readonly`/`unrestricted`） |
...
**无人值守**运行的策略天花板：一次性 `scoot --unattended -e "<goal>"` 钳制，以及未来由
`scoot-edge` 派发的任务。它刻意与 `tools.policy`（交互式默认值）分开，这样抬高交互式策略
绝不会悄悄抬高无人值守天花板。

| 键 | 类型 | 默认 | 含义 |
| --- | --- | --- | --- |
| `max_job_policy` | 字符串 | `"readonly"` | 无人值守任务的天花板：`readonly`（fail-closed）、`guarded`（无人值守时被矫正为 `readonly`，因此什么都没多给）、或 `unrestricted`（高风险、全程审计、需本地签字的跳变，授予写 / 网络） |

天花板**在子进程内对本地 config** 强制，因此命令行或 wire 请求永远只能把策略*降*到它以下，
绝不能抬高。带 `scoot --unattended -e "<goal>"` 时，有效策略为
`correctUnattended(privilegeMin(requested, edge.max_job_policy))`；可选的 `--policy <mode>`
会被钳制到这个天花板。可用 `SCOOT_EDGE_MAX_JOB_POLICY` 按进程覆盖。
