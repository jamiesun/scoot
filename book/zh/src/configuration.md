...
| `SCOOT_BACKEND_PROMPT_CACHE` | `backend.prompt_cache` | 字符串（`off` / `anthropic`） |
...
| `prompt_cache` | string | `off` | prompt 缓存提示模式：`off` 或 `anthropic`。见下。 |
| `extra_body` | table? | unset | 合并进每个请求的额外顶层 JSON 字段。 |

### `prompt_cache`

控制请求体是否携带 prompt 缓存断点，使**稳定指令前缀**（开头的 `system` 段——系统
提示词、工具说明、技能清单；每回合重发）按缓存价计费，而非每回合按全价重算。

- `off`（默认）——不写任何缓存标记，请求体与旧行为**逐字节一致**。OpenAI / vLLM /
  SGLang 会自动缓存稳定前缀，故保持 off（也避免严格后端因未知字段报错）。
- `anthropic`——给开头 `system` 段的 content 打 Anthropic 风格
  `cache_control: {type: ephemeral}` 断点。**仅**在 Anthropic 兼容网关上开启。
  未知值回落 `off`。
...
agent 会先**压缩历史**——保留 system 提示、原始任务与最近若干回合，把更早的工具记录
替换为简短摘要标记——让长 run 得以继续，而非整段中止。仅当压缩后对话记录仍超预算
