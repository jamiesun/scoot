...
var input_buf: [96 * 1024]u8 = undefined;
...
export fn _start() void {
    ...
    const hints = countSensitiveHints(input);

    var out = FixedWriter{ .buf = &output_buf };
    out.write("{\"marker\":\"wasm redactor compressed ");
    out.writeUsize(count);
    out.write(" messages / ");
    out.writeUsize(bytes);
    out.write(" bytes; redaction hints ");
    out.writeUsize(hints);
    out.write("\"}\n");
...
fn countSensitiveHints(input: []const u8) usize {
    return countOccurrences(input, "password") +
        countOccurrences(input, "secret") +
        countOccurrences(input, "token") +
        countOccurrences(input, "api_key") +
        countOccurrences(input, "Bearer ") +
        countOccurrences(input, "sk-");
}

fn countOccurrences(input: []const u8, needle: []const u8) usize {
    if (needle.len == 0) return 0;
    var count: usize = 0;
    var i: usize = 0;
    while (i + needle.len <= input.len) {
        if (startsWith(input[i..], needle)) {
            count += 1;
            i += needle.len;
        } else {
            i += 1;
