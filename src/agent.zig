...
const react_schema =
    \\{"type":"object","properties":{"thought":{"type":"string"},"action":{"type":"string","enum":["bash","file_read","file_write","file_edit","grep","glob","final"]},"action_input":{"type":"string"}},"required":["thought","action","action_input"],"additionalProperties":false}
;
...
/// 模型可选的动作。
pub const Action = enum { bash, file_read, file_write, file_edit, grep, glob, final };
...
fn toolErrorObservation(arena: std.mem.Allocator, err: anyerror) ![]u8 {
    const hint = switch (err) {
        error.MalformedArgs => "action_input 不是合法的参数 JSON。file_read 用 {\"path\":\"...\"}；file_write 用 {\"path\":\"...\",\"content\":\"...\"}；file_edit 用 {\"path\":\"...\",\"old\":\"...\",\"new\":\"...\"}；grep 用 {\"pattern\":\"...\",\"path\":\"...\"}；glob 用 {\"pattern\":\"...\"}。",
