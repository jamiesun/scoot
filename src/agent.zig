...
test "run: readonly 安全档放行 http GET（net_read），黑洞地址命中硬超时观察" {
    const gpa = std.testing.allocator;
    // http GET 属 net_read，只读档应放行 → 执行命中硬超时（黑洞地址）→ 超时观察。
    const s_get =
        \\{"thought":"取数据","action":"http_request","action_input":"{\"method\":\"GET\",\"url\":\"http://10.255.255.1/\"}"}
    ;
    const s_final =
        \\{"thought":"放弃网络","action":"final","action_input":"网络不可达"}
    ;
    var brain = ScriptedBrain{ .steps = &.{ s_get, s_final } };
    var ag = testAgent(&brain, 16);
    ag.policy_mode = .readonly;
    ag.tool_timeout_ms = 300; // 硬超时 300ms，黑洞 connect 须被中断
    ...
    const reply = try ag.run(gpa, &sess);
    defer gpa.free(reply);
    try std.testing.expectEqualStrings("网络不可达", reply);

    // GET 未被 policy_deny（net_read 在 readonly 放行），且产生了超时观察。
    const log = lw.buffered();
    try std.testing.expect(std.mem.indexOf(u8, log, "\"kind\":\"policy_deny\"") == null);
    var saw_timeout = false;
    for (sess.items()) |m| {
        if (std.mem.indexOf(u8, m.content, "超过硬超时") != null) saw_timeout = true;
    }
