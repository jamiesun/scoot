...
const repl_hint = "输入目标后回车开始执行。\"/help\" 查看命令，\"/exit\" 退出。\n";

const repl_help =
    \\Scoot 在「思考→行动→观察」循环里执行你的目标。
    \\
    \\  /help         显示本帮助
    \\  /exit, /quit  退出（会话已自动落盘）
    \\
;
...
/// 为一次人在场/脚本化交互生成独立 transcript id。
/// schedule/daemon job 仍由调用方传入固定 `job-<id>`，保留持续任务的聚合语义。
...
fn runRepl(
    ...
    try out.print(ansi.bold ++ "Scoot {s}" ++ ansi.reset ++ " (model " ++ ansi.cyan ++ "{s}" ++ ansi.reset ++ ", 策略 {s})\n", .{
        scoot.version, cfg.backend.model, cfg.tools.policy,
    });
    if (trace) try out.writeAll(ansi.dim ++ "（--trace 已开启：执行轨迹输出到 stderr）" ++ ansi.reset ++ "\n");
...
fn replLoop(
    ...
    const err_pre = if (color) ansi.bold_red ++ "✗" ++ ansi.reset ++ " " else "✗ ";
    while (true) {
        ...
        const reply = ag.run(backing, sess) catch |err| {
            ...
            if (ag.audit) |lg| lg.log(.system_error, @errorName(err)) catch {};
            try out.print("{s}后端调用失败：{s}（请检查后端是否在运行）\n", .{ err_pre, @errorName(err) });
