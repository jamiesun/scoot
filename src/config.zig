...
pub const Agent = struct {
    ...
    /// 上下文压缩策略：extractive（默认，确定式抽取纪要）/ drop（旧的有损计数标记）。
    compactor: []const u8 = "extractive",
    /// 上下文预算（字节）：跨回合累计的提示历史超过此值时，先按 compactor 压缩历史
    /// （保留 system + 原始任务 + 最近若干回合，见 compressor.zig / issue #71）
    /// 让 run 继续推进；仅当压缩后仍超限才在下次后端调用前 fail-fast（issue #28）。
    /// 0 = 显式关闭（仅受 max_turns 约束）。字节是 token 体量的粗略代理，
...
test "parseFileConfig: context_budget_bytes 可显式设为 0 关闭预算" {
    var arena: std.heap.ArenaAllocator = .init(std.testing.allocator);
    defer arena.deinit();
    const fc = try parseFileConfig(arena.allocator(),
