...
`extra_paths` 不是宽泛读权限；每一项都必须是只放已审查 skill 子目录的专用 skill root。不要把它指向 `$HOME`、`/`、仓库根、`~/.scoot`、运行期 `logs/` / `state/`，或任何可能混有密钥和无关项目文件的目录。
...
警告：过宽的 `extra_paths` 会把这条有意设计的读取边界放大成大面积信息暴露面。任何非专用 skill root 都应视为无效配置，在无人值守运行前必须收窄。
