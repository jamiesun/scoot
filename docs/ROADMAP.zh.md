...
- **绝不搞复杂的云端同步。** 不引入远程数据库，不做端到端加密的状态同步，不与任何特定网络栈强耦合。状态数据严格留在本地。可选、需 opt-in、独立的 `scoot-edge` 伴生程序所做的 append-only 舰队遥测与 schema 化任务派发（见方向七与 [EDGE.zh.md](EDGE.zh.md)）不算云端同步：它绝不做双向状态对账、绝不把中心当作可写真相源、绝不抬高本地策略天花板。
...
- **方向七：可选的舰队边缘代理（`scoot-edge`）。**
  服务于“管理中心需要观测并（可选）向 Scoot 实例舰队派发任务，但不污染 local-first 核心”的取向。该能力收口到一个**可选、独立、默认不安装**的伴生二进制 `scoot-edge`，姿态与 `scoot-wasm` 一致：从不链接进核心，只经公共发射接口（`scoot -e`、`scoot serve` NDJSON、`scoot schedule`、exit code）与只读 `logs/*.jsonl` 驱动 Scoot。默认安装仍是纯核心，无出站连接、无监听。
  边界钉死在 [EDGE.zh.md](EDGE.zh.md)：edge 经 HTTPS（即便内网 VPC 也强制）以逐节点 bearer token 出站拨号连接中心；遥测只追加、绝不回灌本地状态；任务派发只携带 schema 化的 `kind`，其中 goal 是数据、仍由 Scoot 校验；edge 派发任务默认 `readonly`，且天花板中心永远无法抬高。授权模型与红线必须在写任何代码之前签字（E0 阶段）。

## 完成的样子

> 当下面这些**可观察的结果**真实出现时，才算 Scoot 的底盘真正焊死。具体阈值与手段（30 秒、SIGKILL、JSONL 等）是举例，重点是结果本身能被观测到。
...
- **密钥零泄露。** 仅设置环境变量或 `~/.scoot/token`（0600）即可让 Scoot 取到 token；若该文件被改成 group/other 可读，Scoot 拒绝启动并给出明确提示；任何日志、审计、错误信息里都搜不到 token 明文。

## 近期值得做的工作

- 保持后端与运行诊断的可操作性：HTTP 状态码、响应摘录、运行小结、transcript/审计路径、策略拒绝计数都应便于查看。
- 审计可选扩展边界（`mcp_call`、`wasm_tool`、外部 compressor 插件、`serve`）的总超时、输出有界、密钥脱敏与请求作用域分配。
- 让中英文档与 mdBook 页面与实际动作集、稳定 API 边界保持一致。
- 持续加固运行时治理：目录权限、有界保留、审计查询易用性与失败模式的清晰度。
- 仅在 ReACT 可靠性、诊断与扩展边界审计更稳固之后，才着手构建 plan 模式。
