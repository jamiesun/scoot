...
状态：**E1 已基本实现；E2 任务派发已实现。** E0 边界已经签字，当前已有一个独立、显式 opt-in 的 `scoot-edge` 伴生程序，需通过 `zig build -Dedge=true` 构建。它可以在本地生成一次仅报告态的 status 心跳，用逐节点 bearer token 把该心跳 `post-once` 到 HTTPS endpoint，并能以 **`run` 持续心跳循环**按固定间隔向外拨出上报：遇到瞬时失败时采用有界的抖动退避（绝不让循环崩溃，也绝不开启监听端口）。心跳还可携带一个**显式 opt-in、仅作建议的 `node` 能力描述符**（`--report-capabilities`），供能力感知路由使用。一个刻意命名的 `--allow-insecure-http` 开关只用于本地 / dev loopback 环境对接纯 HTTP center；即便带上该开关，非 loopback 的 `http://` URL 也会被拒绝，生产仍保持 HTTPS-only。`scoot-edge dispatch`（也可通过 `run --enable-jobs` 达到同样效果）现在会轮询一次 `GET` 任务 lease，通过 `scoot --unattended -e "<goal>"` 执行 0..N 个任务、cwd 被限制在 `edge.job_root` 内，并把 `job_event` 上报回去——全程带有有界、幂等、可追溯的落地记录（#186）。audit 正文搬运仍然刻意未实现，并继续受下方前置条件门控。
...
- **强制 HTTPS，即便在内网。** 传输由服务端 TLS 加密：edge 校验中心证书。这**不是 mTLS**——客户端身份由 token（见下）携带，而非客户端证书，从而把证书管理压缩到单张服务端证书。E1 的 `post-once` 命令有一个显式的 `--allow-insecure-http` 逃生口，只用于本地 / dev loopback 测试；非 loopback `http://` URL 即使带开关也会拒绝。它不是生产传输模式。
...
### node 描述符（身份与能力，opt-in）

上面那条裸心跳只表达存活。要做能力感知的派发，中心得知道一个节点*是干什么用的*，所以 status body 可以携带一个可选的 `node` 描述符——遵循与 audit 搬运完全一致的 fail-closed 条款：**默认关闭**（`edge.report_capabilities`），因为一份能力 / skill 清单同样是一个舰队侦察面；而且**仅供参考，绝不是授权**。
...
- **广播不等于授权。** 描述符只帮中心*决定该派什么*；执行仍由本地 `policy_ceiling` 门控。声明一项能力绝不会扩大 edge 实际会做的事，而多报或少报都退化为任务被拒，绝不退化为不安全执行。
- **三个来源。** `labels` 是运营方在本地 config 声明的（`edge.labels`），用于表达中心无从推断的路由意图；`os` / `arch` / `tools` / `max_job_policy` 是自动导出的本地事实；`skills` 复用既有的 skill 清单——已安装 skill 的*名字*就是节点自我描述的专注信号（skill 指令正文绝不外发）。
- **默认关闭，更丰富则 opt-in。** 裸 status 只上报存活；任何超出存活的东西——audit 正文、能力 / 身份元数据——都是一次显式的本地 opt-in。

### audit_batch（append-only 日志搬运）

数据源：只读 `logs/*.jsonl`。

**把 audit 搬出本机是一次数据搬移决策，不是 no-op。** 约束 7 对*已知*的密钥值是强制生效的：解析出的后端 token，以及任何名字看起来像密钥的环境变量的值，都会在写入 `logs/audit.jsonl` 之前先从 `msg` 正文中脱敏（issue #189）。但它**不**保证 audit `msg` 正文不含*所有*敏感内容——已知密钥值之外，`observation` 与 `tool_call` 事件仍会原样携带工具输出：`file_read` 的文件内容、`bash` 的命令输出、`http_request` 的响应体，都可能带有 Scoot 无法识别为密钥的敏感数据。所以 `audit_batch` 会把一次 run 观测到的一切搬到中心。因此 edge 把 audit 搬运按 fail-closed 处理：
...
- **`file_gen` 现已在核心中落地**（issue #187），解除了本节此前所依赖的那个 E1 硬前置。`src/audit.zig` 的 `rotateGenerational` 取代了旧的单备份 `audit.jsonl → audit.jsonl.1`（每次轮转都会删除旧的 `.1`，若一个节点在两次 edge 轮询之间轮转两次，中间那段会永久丢失）。现在每次轮转都会把活跃文件退休为一个单调编号的 `audit.jsonl.<gen>`，代数计数器持久化在 `audit.jsonl.gen` sidecar 里，因此能在进程重启后存活。最多保留 `[audit].max_retained_generations`（默认 8）个已退休代；只有超过这个上限时才会淘汰最旧的一个——且该次淘汰会以 `{gap_from, gap_to, ts}` 的形式持久记录进 `audit.jsonl.gaps.jsonl`（可通过 `scoot doctor` 读取，一旦曾记录过任何 gap 就会告警），而不是悄悄消失。
- **这是核心侧的保留前置，并非 audit 搬运本身。** 游标里字节偏移的那一半（上面的 `byte_from`/`byte_to`/`seq_to`）以及 wire 层的 `audit_gap` 标记，仍然是尚未落地的 E1 搬运工作——`edge.ship_audit` / `edge.audit_ship_kinds` / `edge.audit_redact` 目前只是文档里设想的 config key，`src/edge_main.zig` 里还没有任何实现。#187 解除的是"搬运永远不可能 at-least-once"的那个根因：核心不再在有人读取之前销毁一个已轮转的代，任何最终被上限淘汰的代都会留下持久痕迹，而不是一个静默的空洞。
- **audit 搬运本身仍未实现，因此仍处于关闭状态。** 落地这个保留前置并不会打开 `audit_batch`；`src/edge_main.zig` 里仍然没有 `edge.ship_audit` config、拨出路径或 ack 协议。这让 E1 的最终投递承诺保持诚实，而不是在搬运代码本身尚不存在时就宣称 at-least-once。
- 在 VPC 部署下，E1 可保持为简单的周期性 `POST /telemetry`。在 E2 引入任务派发之前，edge 无需长轮询。

## 阶段 E2 —— schema 化、幂等的任务派发

**已实现（#186）。** `scoot-edge dispatch` 执行一次 lease-poll-and-execute 循环；`scoot-edge run --enable-jobs` 把同一循环并入心跳循环。两者都要求 `--job-root` 与 `--lease-url`（由与 `--center-url`/token 相同的 config 门控校验：缺失或不安全会以退出码 `2` 终止，绝不静默跳过）。中心通过 edge 出站发起的 `GET` lease 下发任务：
...
中心通过把任务与节点最近一次的 `node` 描述符（其 `capabilities` 与 `labels`）相匹配来路由；edge 不在 wire 上协商能力。节点无法满足的任务以 `no_matching_capability` 被拒，于是能力不匹配退化为拒绝，绝不退化为不安全执行。
...
| 阶段 | 中心能做什么 | 风险 | 建议 |
| --- | --- | --- | --- |
| E1 仅报告 | 默认只有 `status` 心跳；audit 日志搬运仅在显式开启时 | 较低，但 audit 搬运会把 observation 数据（文件内容、命令输出）搬出本机 | 先发 `status`；核心侧轮转现已对 shipping 感知（#187），但在 E1 拨出 / ack 路径本身落地之前，仍保持 audit 搬运关闭 |
| E2 任务派发 | 派发由 edge 经 Scoot 启动的任务 | confused deputy（edge 可能被人当枪使） | 仅在显式 config + 策略天花板之后 |

**已签字的默认值：**

- **Edge 派发任务默认 `readonly`，由钳制强制——不是自动继承的。** 调度器的 `guarded → readonly` 纠正在 `schedule.zig` 里，并**不**作用于一条裸的 `scoot -e`。子进程内的无人值守钳制现已作为 `scoot --unattended -e "<goal>"` 在核心落地（读取本地 `edge.max_job_policy` 天花板，默认 `readonly`），因此 readonly 默认现已有支撑；E2 任务派发经它接线。
...
- **任何能力声明都不授予权限。** `node` 描述符是声明式的、仅供参考的路由元数据；本地 `policy_ceiling` 仍门控每一个任务，而 `edge.report_capabilities` 是 opt-in。广播一项能力绝不会拓宽 edge 实际会执行的范围。
